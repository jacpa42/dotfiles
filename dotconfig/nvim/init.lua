...
autocmd({ "FileType" }, {
	pattern = "zig",
	callback = function(args)
		local compile = "zig build --summary none --color off --error-style minimal -freference-trace=20"
		local test = "zig build test --summary none --color off --error-style minimal -freference-trace=20"
...
map("n", "<m-l>", "<cmd>TmuxNavigateRight<cr>", { noremap = true, silent = true })
...
map("n", "<leader>ft", "<cmd>Telescope diagnostics<cr>", { desc = "search trouble for document" })
map("n", "<leader>t", "<cmd>Telescope lsp_type_definitions<cr>", { desc = "search type definition" })
